...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const wrapContrastText = () => {
      if (textWrapped) return;

      const elements = Array.from(document.querySelectorAll(".contrast-text"));
      ...
      const walk = (node) => {
        Array.from(node.childNodes).forEach((child) => {
          ...
          if (
            ...
            child.tagName !== "BR" &&
            !child.classList.contains("contrast-char")
          ) {
            walk(child);
          }
        });
      ...
      elements.forEach((element) => {
        if (element.dataset.contrastWrapped === "true") return;

        walk(element);
        element.dataset.contrastWrapped = "true";
      });
    ...
    const updateContrast = () => {
      ...
      const chars = Array.from(document.querySelectorAll(".contrast-char"));
      ...
      chars.forEach((char) => {
        ...
        const luminance = getGraphicLuminanceAtPoint(x, y);

        char.style.color =
          typeof luminance === "number" && luminance > 0.54
            ? "#050505"
            : "#ffffff";
      });
    };
    ...
    const animate = () => {
      ...
      if (graphic) {
        graphic.style.transform = `translate3d(-50%, ${currentGraphicY.toFixed(2)}px, 0)`;
      }
